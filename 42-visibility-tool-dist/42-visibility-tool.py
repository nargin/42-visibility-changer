...
def main():
    ...
    try:
        ...
        if not projects_42:
            print("Error: No 42 projects found in your repositories")
        ...
        for i, repo in enumerate(projects_42, 1):
            status = "private private" if repo['private'] else "public public"
        ...
        print("   1. Make repositories private private")
        print("   2. Make repositories public public")
        print("   3. Exit")
        ...
        for i, repo in enumerate(available_repos, 1):
            current_status = "private private" if repo['private'] else "public public"
                ...
                try:
                    ...
                    for num in numbers:
                        if 1 <= num <= len(available_repos):
                            ...
                            print(f"Error: Invalid number: {num} (valid range: 1-{len(available_repos)})")
                    
                    if selected_repos:
                        selected_names = [repo['name'] for repo in selected_repos]
                        print(f" Selected: {', '.join(selected_names)}")
                    else:
                        print("Error: No valid repositories selected")
                        
                except ValueError:
                    print("Error: Invalid input. Enter numbers separated by spaces, 'all', or press Enter")
        
        if not selected_repos:
            print("Error: No repositories selected. Exiting.")
            input("Press Enter to exit...")
            return
        ...
        if confirm in ['y', 'yes']:
            ...
            print("Error: Operation cancelled - no changes made")
